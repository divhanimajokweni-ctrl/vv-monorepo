...
import { members } from "@ubuntu/db/schema";
import {
  poolContributions,
  domainEvents,
  projections,
...
import { villageMembers } from "@ubuntu/db/schema-village";
...
export async function assertVillageMembership(memberId: string, villageId: string): Promise<void> {
  const membership = await db
    .select({ id: villageMembers.id })
    .from(villageMembers)
    .where(
      and(
        eq(villageMembers.userId, memberId),
...
export async function emitContributionEvent(
  ...
  if (!event) {
    throw new SpineError("EVENT_EMIT_FAILED", "Failed to emit contribution event", { memberId, villageId });
  }
...
export async function assertDashboardReflectsProjection(villageId: string): Promise<{
  ...
  if (!p) {
    throw new SpineError("PROJECTION_UNDEFINED", `Projection found but undefined for village '${villageId}'.`, { villageId });
  }
  return { lastEventId: p.lastEventId, refreshedAt: p.refreshedAt };
}
...
export async function writeAuditTrace(
  ...
  if (!trace) {
    throw new SpineError("AUDIT_WRITE_FAILED", "Failed to write audit trace", { eventId, action });
  }
